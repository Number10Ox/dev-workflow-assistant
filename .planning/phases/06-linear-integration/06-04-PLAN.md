...
<task type="auto">
  ...
  <action>
Update registry.js by ADDING the following NEW fields to the existing RUNTIME_FIELDS array (preserve all existing fields like status, linear_id, linear_url, pr_url, completed_at, created_at):

ADD these new fields to RUNTIME_FIELDS:
...
- `drift_events` - Array of drift events (from Phase 5)

The updated RUNTIME_FIELDS should look like:
```javascript
const RUNTIME_FIELDS = [
  'status',
  'linear_id',        // Kept for backwards compatibility
  'linear_issue_id',  // NEW: Linear UUID (for fast lookup)
  'linear_identifier', // NEW: Human-readable (e.g., "ENG-123")
  'linear_url',
  'linear_external_id', // NEW: Our externalId stored in Linear
  'linear_project_id',  // NEW: Resolved container/project ID
  'dwa_sync_hash',      // NEW: Last sync fingerprint
  'pr_url',
  'completed_at',
  'created_at',
  'drift_events'       // NEW: Array of drift events (from Phase 5)
];
```
...
console.log('RUNTIME_FIELDS includes linear_issue_id:', reg.RUNTIME_FIELDS.includes('linear_issue_id'));
console.log('RUNTIME_FIELDS includes dwa_sync_hash:', reg.RUNTIME_FIELDS.includes('dwa_sync_hash'));
console.log('RUNTIME_FIELDS preserves status:', reg.RUNTIME_FIELDS.includes('status'));
console.log('RUNTIME_FIELDS preserves pr_url:', reg.RUNTIME_FIELDS.includes('pr_url'));
"
```
  </verify>
  <done>
registry.js RUNTIME_FIELDS includes all original fields PLUS: linear_issue_id, linear_identifier, linear_external_id, linear_project_id, dwa_sync_hash, drift_events. New updateLinearFields() function exported.
  </done>
</task>
...
1. Registry has Linear fields:
   ```bash
   grep -n "linear_issue_id\|dwa_sync_hash" \
     /Users/jedwards/workspace/JobPrep/DevEx/dev-workflow-assistant/src/parser/registry.js
   ```
...
<success_criteria>
- registry.js RUNTIME_FIELDS includes original fields PLUS: linear_issue_id, linear_identifier, linear_external_id, linear_project_id, dwa_sync_hash, drift_events
