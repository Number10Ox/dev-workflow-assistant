...
must_haves:
  truths:
    - "TDD template contains all 8 sections (Objectives through Revision History)"
...
<objective>
Create TDD template and scaffolding utility for Technical Design Documents.

Purpose: Enable users to scaffold a TDD linked to their feature spec, with industry-standard sections for objectives, architecture, decisions (ADR format), guardrails, risks, and test strategy.
Output: TDD Handlebars template, scaffolding function, and comprehensive tests.
</objective>
...
<task type="auto">
  <name>Task 1: Create TDD Handlebars template</name>
  <files>templates/tdd-v1.hbs</files>
  <action>
Create templates/tdd-v1.hbs with the following structure from 02-RESEARCH.md:
...
Template variables:
- `{{feature_id}}` - From spec frontmatter (e.g., FEAT-2026-123)
- `{{title}}` - Feature title from spec
- `{{spec_path}}` - Relative path from TDD to spec (e.g., ../../feature-spec.md)
- `{{created_date}}` - Today's date in YYYY-MM-DD format
- `{{owner}}` - Git user name
  </action>
  <verify>
Read templates/tdd-v1.hbs and confirm:
- YAML frontmatter with tdd_schema_version: v1.0
- All 8 sections present (Objectives, Architecture, Decision Log, Guardrails, Risks, Test Strategy, Implementation Notes, Revision History)
- ADR format example in Decision Log section
- Handlebars placeholders use correct variable names
  </verify>
  <done>TDD template exists with all 8 sections and ADR format</done>
</task>

<task type="auto">
  <name>Task 2: Create scaffold-tdd.js utility</name>
  <files>src/scaffolding/scaffold-tdd.js</files>
  <action>
Create src/scaffolding/scaffold-tdd.js with scaffoldTDD function:
...
- Placeholder leakage detection (same pattern as scaffold.js)
  ...
  <verify>
Read src/scaffolding/scaffold-tdd.js and confirm:
- Function is exported as scaffoldTDD
- Creates docs/tdds/ directory
- Uses Handlebars to render tdd-v1.hbs
- Updates feature.json with tdd_path
- Updates spec frontmatter with tdd_path
- Uses atomic file writes
  </verify>
  <done>scaffold-tdd.js creates TDD files with bidirectional linking</done>
</task>
...
Run `npm test` - all tests pass including new scaffold-tdd tests.
  ...
  <done>TDD scaffolding tests exist and pass, covering all requirements</done>
</task>

</tasks>

<verification>
1. `npm test` passes (all scaffold and scaffold-tdd tests)
2. Manual verification:
   ...
   # Check feature.json updated
   cat "$TESTDIR/.dwa/feature.json"

   # Check spec frontmatter updated
   cat "$TESTDIR/feature-spec.md" | head -15

   rm -rf "$TESTDIR"
   ```
3. TDD file has all 8 sections
4. Bidirectional linking works (tdd_path in spec, spec_path in TDD)
</verification>
